<?php 
if(!class_exists('user_role_ajax_call_class')) 
{
    class user_role_ajax_call_class
    {
		...
		public function init()
        {
			...
			add_action( 'admin_enqueue_scripts', 'enqueue_user_role_ajax_booklyapp' );
            function enqueue_user_role_ajax_booklyapp($hook) {				
                wp_enqueue_script('ajax-script-aur', plugins_url( '../js/user-role-ajax.js?v='.rand(), __FILE__ ), array('jquery'));	
			...
			// add category data
			add_action('wp_ajax_add_aur_role', 'add_user_role_callback');
       		function add_user_role_callback() {
				global $wpdb;
