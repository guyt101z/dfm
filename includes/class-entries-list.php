...
class DynamicFormMaker_Form_Entries_List extends WP_List_Table {
	...
	function column_form( $item ){
		...
		elseif ( $this->get_entry_status() && 'trash' == $this->get_entry_status() ) {
			$actions['restore'] = sprintf( '<a href="?page=%s&action=%s&entry=%s">%s</a>', $_REQUEST['page'], 'restore', $item['entry_id'], __( 'Restore', 'dynamic-form-maker' ) );
			$actions['delete'] = sprintf( '<a href="?page=%s&action=%s&entry=%s">%s</a>', $_REQUEST['page'], 'delete', $item['entry_id'], __( 'Delete Permanently', 'dynamic-form-maker' ) );
		}
	...
	function get_columns(){
		$columns = array(
			...
			'form' 			=> __( 'Form' , 'dynamic-form-maker'),
			'subject' 		=> __( 'Email Subject' , 'dynamic-form-maker'),
			...
			'date' 			=> __( 'Date Submitted' , 'dynamic-form-maker'),
			'entry_id'		=> __( 'Entry ID' , 'dynamic-form-maker'),
		);
	...
	function get_bulk_actions() {
		...
		if ( !$this->get_entry_status() || 'all' == $this->get_entry_status() )
			$actions['trash'] = __( 'Move to Trash', 'dynamic-form-maker' );
		elseif ( $this->get_entry_status() && 'trash' == $this->get_entry_status() ) {
			$actions['restore'] = __( 'Restore', 'dynamic-form-maker' );
			$actions['delete'] = __( 'Delete Permanently', 'dynamic-form-maker' );
		}
	...
	function extra_tablenav( $which ) {
		...
		if ( 'top' == $which ) {
			...
			echo '<select id="form-filter" name="form-filter">
				<option value="-1"' . selected( $this->current_filter_action(), -1 ) . '>' . __( 'View all forms' , 'dynamic-form-maker-pro') . '</option>';
			...
			echo '</select>
				<input type="submit" value="' . __( 'Filter' , 'dynamic-form-maker-pro') . '" class="button-secondary" />
				</div>';
		}
	}
