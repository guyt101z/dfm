jQuery(document).ready(function($) {
	...
	if( window.VfbAdminPages ) {
		...
		if ( pagenow == obj['dfm'] ) {
			...
			postboxes.add_postbox_toggles( obj['dfm'] );
		}
	}
	...
	$( document ).on( 'mouseenter mouseleave', '.dfm-tooltip', function( e ) {
		...
		if( e.type == 'mouseenter' ) {
			...
			$( this ).append( '<div class="dfm-tooltip-popup"><h3>' + tip_title + '</h3><p>' + tip + '</p></div>' );
			...
			$( this ).find( '.dfm-tooltip-popup' ).css({left:width + 22});

			// Set a timer for hover intent
			tooltip_timeout = setTimeout( function(){
				$( '.dfm-tooltip-popup' ).fadeIn( 300 );
			}, 500 );
		}
		else {
			// Add the title back
	...
	$( document ).on( 'click', 'a.dfm-add-option', function( e ) {
		e.preventDefault();

		var clones = $( this ).parent().siblings( '.dfm-cloned-options' ),
	...
	$( document ).on( 'click', 'a.item-edit', function( e ){
		...
		if ( item.hasClass( 'dfm-menu-item-inactive' ) ) {
			item.removeClass( 'dfm-menu-item-inactive' )
				.addClass( 'dfm-menu-item-active' );
		}
		else {
			item.removeClass( 'dfm-menu-item-active' )
				.addClass( 'dfm-menu-item-inactive' );
		}
	});
    ...
    function is_fieldset_first( item ) {
	    if ( 'FIELDSET' !== item )
	    	$( '#dfm-fieldset-first-warning' ).show();
	    else
	    	$( '#dfm-fieldset-first-warning' ).hide();
    }

	// !Nest and Sort fields
	$( '#dfm-menu-to-edit' ).nestedSortable({
		listType: 'ul',
		maxLevels: 3,
		handle: '.dfm-menu-item-handle',
		placeholder: 'dfm-sortable-placeholder',
		...
		stop: function( event, ui ){
			...
			var sorted_first_item = $( '#dfm-menu-to-edit .item-type:first' ).text();
			...
			opts = {
				...
				cache: false,
				data: {
					action: 'dynamic_form_maker_sort_field',
	...
	$( '#form-items .dfm-draggable-form-items' ).click( function( e ) {
		e.preventDefault();
	...
	$( '#form-items .dfm-draggable-form-items' ).click( function( e ) {
		...
		var d = $( this ).closest( 'form' ).serializeArray(),
			...
			previous = $( '#dfm-menu-to-edit li.ui-state-disabled:first' ).attr( 'id' ).match( new RegExp( /(\d+)$/g ) )[0];

		$( 'img.waiting' ).show();

		$.post( ajaxurl,
			{
				action: 'dynamic_form_maker_create_field',
				data: d,
			...
			$( response ).hide().insertBefore( '#dfm-menu-to-edit li.ui-state-disabled:first' ).fadeIn();
		});
	});
	...
	$( document ).on( 'click', 'a.item-delete', function( e ) {
		...
		$.post( ajaxurl,
			{
				action: 'dynamic_form_maker_delete_field',
				form: data['form'],
				field: data['field'],
				child_ids: childs,
				parent_id: parent,
	...
	$( '#form-settings-button' ).click( function(e){
		...
		$.post( ajaxurl,
			{
				action: 'dynamic_form_maker_form_settings',
				form: form_id,
				status: state,
				page: pagenow
			}
	...
	var dfm_forms_changed = false;
	$( '#dfm-form-builder-management input, #dfm-form-builder-management select, #dfm-form-builder-management textarea' ).change( function(){
		dfm_register_change();
	});

	function dfm_register_change() {
		dfm_forms_changed = true;
	}

	window.onbeforeunload = function(){
		if ( dfm_forms_changed )
			return 'The changes you made will be lost if you navigate away from this page.';
	};

	$( document ).on( 'submit', '#dynamic-form-maker-update', function() {
		window.onbeforeunload = null;
	});

	// !Sticky sidebar
	if ( $( '.columns-2 #side-sortables' ).length > 0 ) {
	    var sidebar = $( '#dfm_form_items_meta_box' ),
	...
	// !Field Types tabs
	$( '.dfm-field-types' ).click(function( e ){
		e.preventDefault();

		$( '#dfm-field-tabs li' ).removeClass( 'tabs' ); //Remove any "active" class
	...
	$( '#dynamic-form-maker-update' ).validate({
		rules: {
			'form_email_to[]': {
				email: true
			},
	...
	$( '#dynamic-form-maker-new-form' ).validate();
	...
	$( '#dfm-export-entries-forms' ).change( function(){
		...
		$.get( ajaxurl,
			{
				action: 'dynamic_form_maker_export_load_options',
				id: id,
				count: count,
				page: pagenow
			}
